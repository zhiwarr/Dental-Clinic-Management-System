...
// Get variable from the previous page (index.php)
$patientUsername = $_SESSION['patientUsername'];

// Patient ID and patient info details
$pIDStmt = $dbconn->prepare("SELECT patient_id FROM user_account WHERE username = :patientUsername");
$pIDStmt->bindParam(':patientUsername', $patientUsername, PDO::PARAM_STR);
$pIDStmt->execute();
$pID = $pIDStmt->fetchColumn();

$pSinStmt = $dbconn->prepare("SELECT sin_info FROM Patient WHERE patient_id = :pID");
$pSinStmt->bindParam(':pID', $pID, PDO::PARAM_INT);
$pSinStmt->execute();
$pSin = $pSinStmt->fetchColumn();

$pNameStmt = $dbconn->prepare("SELECT name FROM Patient_info WHERE patient_sin = :pSin");
$pNameStmt->bindParam(':pSin', $pSin, PDO::PARAM_STR);
$pNameStmt->execute();
$pName = $pNameStmt->fetchColumn();

$pGenderStmt = $dbconn->prepare("SELECT gender FROM Patient_info WHERE patient_sin = :pSin");
$pGenderStmt->bindParam(':pSin', $pSin, PDO::PARAM_STR);
$pGenderStmt->execute();
$pGender = $pGenderStmt->fetchColumn();

// Continue similarly for other details...

// Patient records details
$patientRecordsStmt = $dbconn->prepare("SELECT record_id, patient_details FROM Patient_records WHERE patient_id = :pID");
$patientRecordsStmt->bindParam(':pID', $pID, PDO::PARAM_INT);
$patientRecordsStmt->execute();
$patientRecords = $patientRecordsStmt->fetchAll(PDO::FETCH_ASSOC);

// Continue similarly for other queries...

// Reviews query (all reviews on the website)
$reviewsStmt = $dbconn->prepare("SELECT * FROM Review ORDER BY review_id DESC");
$reviewsStmt->execute();
        ...
        <div class="row">
            ...
            <div class="profile-info col-md-9">
                ...
                <div class="panel" id="patient_appointments">
                    ...
                    <form action="patient_landing.php#patient_appointments" method="post"
                    ...
                    <div class="panel-body bio-graph-info">
                       ...
                       switch ($sorting) {
                        case "apptSort1":
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY appointment_id DESC";
                            break;
                        case "apptSort2":
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY appointment_id";
                            break;
                        case "apptSort3":
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY dentist_id";
                            break;
                        case "apptSort4":
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY date_of_appointment DESC";
                            break;
                        case "apptSort5":
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY date_of_appointment";
                            break;
                        case "apptSort6":
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY appointment_type";
                            break;
                        case "apptSort7":
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY appointment_status";
                            break;
                        default:
                            $apptSortQuery = "SELECT * FROM Appointment WHERE patient_id = :pID ORDER BY appointment_id DESC";
                    }
                    ...
                    $apptSortStmt = $dbconn->prepare($apptSortQuery);
                    $apptSortStmt->bindParam(':pID', $pID, PDO::PARAM_INT);
                    $apptSortStmt->execute();
                    
                    // Fetch the results
                    $patientAppointments = $apptSortStmt->fetchAll(PDO::FETCH_ASSOC);
                ...
                <div class="panel" id="patient_invoices">
                    ...
                    <div class="panel-body bio-graph-info">
                        <p><i class="fa fa-question-circle"></i> Invoices are created after an Appointment Procedure is
                            completed
                        </p>
                        <table id="appointments_grid" class="table" width="100%" cellspacing="0">
                            <thead>
                    ...
                    <div class="panel-body bio-graph-info">
                        ...
                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                            <fieldset>
                                ...
                                <input type="radio" name="payment_method" value="nopayment" checked="true"> No
                                Payment<br>
                                <input type="radio" name="payment_method" value="yespayment"> Make a Payment<br>
                            </fieldset>
                            <br><br>
                            <button class="btn btn-lg btn-primary btn-block btn-warning" type="submit"
                                name="login">Proceed to
                                payment</button>
                        </form>
                    </div>
                </div>
                ...
                <form action="patient_landing.php#patient_reviews_submit" method="post"
                    action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                    <div class="panel" id="patient_reviews_submit">
                        <!-- Comment box -->
                        <textarea name="comment" placeholder="Leave us an anonymous review! (Max. 255 characters)"
                            rows="4" class="form-control input-lg p-text-area" maxlength="255"
                            style="color:#6C6C6C"><?php echo $comment;?></textarea>
                        ...
                        <footer class="panel-footer">
                            ...
                            <ul>
                                <!-- This makes it look nicer, but it covers a bit of the Submit button... style="position:relative; right:40px; top:8px; z-index: 1" -->
                                <!-- <li> -->
                                <!-- Dentist Name Working -->
                                <label for="dentistName">Doctor:<span class="error">*
                                        <?php echo $dentistNameErr;?></span></label>
                                <select name="dentistName" id="dentistName">
                                    ...
                                    <option value="<?php echo $apptDentistNames['name'];?>">
                                        <?php echo $apptDentistNames['name'];?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                                <!-- </li> -->
                                ...
                                <!-- <li> -->
                                <label for="professionalism">Professionalism:<span class="error">*
                                        <?php echo $professionalismErr;?></span></label>
                                ...
                                <!-- <li> -->
                                <label for="communication">Communication:<span class="error">*
                                        <?php echo $communicationErr;?></span></label>
                                ...
                                <!-- Cleanliness Working -->
                                <!-- <li> -->
                                <label for="cleanliness">Cleanliness:<span class="error">*
                                        <?php echo $cleanlinessErr;?></span></label>
                                ...
                                <!-- </li>
                                    <li> -->
                                <!-- <br> -->
                                <label for="procedure_id">Procedure:<span class="error">*
                                        <?php echo $procedureIDErr;?></span></label>
                                <select name="procedure_id" id="procedure_id">
                                    <option value="">-</option>
                                    <?php 
                                            // Appointment_Procedure query
                                          // Query to fetch appointment procedures for a patient ordered by date_of_procedure DESC
$apptProceduresQuery = "SELECT * FROM Appointment_procedure WHERE patient_id = :pID ORDER BY date_of_procedure DESC";

// Prepare and execute the query
$apptProceduresStmt = $dbconn->prepare($apptProceduresQuery);
$apptProceduresStmt->bindParam(':pID', $pID, PDO::PARAM_INT);
$apptProceduresStmt->execute();

// Fetch the results
$apptProcedures = $apptProceduresStmt->fetchAll(PDO::FETCH_ASSOC);
...
// Move the $_POST user inputs into PHP variables for cleaner code
$dentistNameInput = $_POST["dentistName"];
$professionalismInput = $_POST["professionalism"];
$comment = str_replace("'", "''", $_POST["comment"]); // replace ' with '' for Postgres insertion
...
// Query to insert a review using prepared statements
$reviewQuery = "INSERT INTO Review (dentist_name, review_description, professionalism, communication, cleanliness, date_of_review, procedure_id) 
                VALUES (:dentistName, :comment, :professionalism, :communication, :cleanliness, :reviewDate, :procedureID)";

// Prepare the query
$insertReviewStmt = $dbconn->prepare($reviewQuery);

// Bind parameters
$insertReviewStmt->bindParam(':dentistName', $dentistNameInput, PDO::PARAM_STR);
$insertReviewStmt->bindParam(':comment', $comment, PDO::PARAM_STR);
$insertReviewStmt->bindParam(':professionalism', $professionalismInput, PDO::PARAM_INT);
$insertReviewStmt->bindParam(':communication', $communicationInput, PDO::PARAM_INT);
$insertReviewStmt->bindParam(':cleanliness', $cleanlinessInput, PDO::PARAM_INT);
$insertReviewStmt->bindParam(':reviewDate', $reviewDate, PDO::PARAM_STR);
$insertReviewStmt->bindParam(':procedureID', $procedureIDInput, PDO::PARAM_INT);

// Execute the query
$insertReviewResult = $insertReviewStmt->execute();

// Check if the insertion was successful
if (!$insertReviewResult) {
    print_r($insertReviewStmt->errorInfo()); // Output detailed error information
    echo "<h5>There was an error adding the review</h5>";
} else {
    echo "Review Added Successfully!<br><br>";
    echo "Your response was submitted on " . $reviewDate . "<br><br><br><br>";
}
